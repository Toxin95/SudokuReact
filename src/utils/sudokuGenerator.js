...
class SudokuGenerator {
    ...
    constructor() {
        this.SQUARES = this._cross(this.ROWS, this.COLS);             // Square IDs

        this.UNITS = this._get_all_units(this.ROWS, this.COLS);               // All units (row, column, or box)
        this.SQUARE_UNITS_MAP = this._get_square_units_map(this.SQUARES, this.UNITS);    // Squares -> units map
        this.SQUARE_PEERS_MAP = this._get_square_peers_map(this.SQUARES, this.SQUARE_UNITS_MAP);    // Squares -> peers map
        ...
        for (var si in shuffled_squares) {
            ...
            if (single_candidates.length >= difficulty &&
                ...
                for (let i in this.SQUARES) {
                    ...
                    if (candidates[currentSquare].length === 1) {
                        board += candidates[currentSquare];
                        givens_idxs.push(i);
                    } else {
                ...
                if (nr_givens > difficulty) {
                    ...
                    for (let i = 0; i < nr_givens - difficulty; ++i) {
                        ...
                        board = board.substring(0, target) + this.BLANK_CHAR +
